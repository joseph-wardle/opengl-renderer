...
struct HelloCube {
    HelloCube() {
        ...
        if (core::length(rotation_axis_) < 0.001f) {
            rotation_axis_ = core::Vec3{0.0f, 1.0f, 0.0f};
        } else {
            rotation_axis_ = core::normalize(rotation_axis_);
        }
    ...
    void on_render() {
        ...
        const core::Mat4 model = core::mul(
            core::translate(core::Mat4{1.0f}, core::Vec3{0.0f, bounce_offset_, 0.0f}),
            core::rotate(core::Mat4{1.0f}, rotation_angle_, rotation_axis_)
        );
        const core::Mat4 view  = core::lookAt(core::Vec3{0.0f, 0.0f, 3.0f}, core::Vec3{0.0f}, core::Vec3{0.0f, 1.0f, 0.0f});
        const core::Mat4 proj  = core::perspective(core::radians(45.0f), aspect_ratio_, 0.1f, 100.0f);
        const core::Mat4 mvp   = core::mul(core::mul(proj, view), model);

        shader_.use();
        if (auto loc = gpu::gl::get_uniform_location(shader_.id(), "uMVP"); loc != -1) {
            gpu::gl::set_uniform_mat4(loc, core::value_ptr(mvp));
        }
